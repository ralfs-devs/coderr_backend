"""Serializers handling evaluation records and scoring inputs for business profiles."""
...
class ReviewsSerializer(serializers.ModelSerializer):
    ...
    def validate_rating(self, value):
        """Validates that the provided rating score stays within the permitted scale limits.

        Args:
            value (int): The numeric rating score value to be verified.

        Returns:
            int: The verified numeric score value.

        Raises:
            ValidationError: If the rating score falls outside the range of 1 to 5.
        """
        if not (1 <= value <= 5):
